...
mainDir <- "~./Simulation/Simulation1"
...
for(l in 1:length(perc)){
  for(j in 1:length(phi)){
    
    subDir <- paste("Cens",perc[l],"phi",phi[j],sep = "_")
    CreateDir(mainDir,subDir)
    
    funcSemi <- function(tt) 2*sin(0.25*pi*tt)
    beta0 <- 2
    beta1 <- -1.5
    betas <- c(beta0,beta1)
    sigmae <- 0.55
    ...
    theta_ar <- matrix(0,nSim,14)
    theta_unc <- matrix(0,nSim,12)
    lambda_ar <- lambda_unc <- matrix(0,nSim,1)
    dp_Infbf_ar <- dp_Infbf_unc <- matrix(0,nSim,8)
    criterio_ar <- criterio_unc <- matrix(0,nSim,2)
    AIC_criterio_ar <- AIC_criterio_unc <- matrix(0,nSim,1)
    ...
    for(i in 1:nSim)
    {
      ...
      EST_unc <- nsmec(y=y, cc=cc, x=x, z=z, tt=tt, ttc=ttc, nj=nj, LL=LL, LU=LU, Ns=Ns, initial=initial, struc="unc", lambda.fixed=FALSE, iter.max=300,precision=0.0001)
      
      theta_unc[i,] <- c(EST_unc$beta1, EST_unc$ff, EST_unc$sigmae,EST_unc$dd)
      lambda_unc[i,] <-  EST_unc$lambda
      criterio_unc[i,] <- c(EST_unc$loglikp,EST_unc$loglik)
      AIC_criterio_unc[i,] <- c(EST_unc$AICp)
      dp_Infbf_unc[i,] <- sqrt(diag(solve(EST_unc$Infbetasff)))
      
      write.table(theta_unc,file="theta_cens_unc.txt",sep=" ",row.names=F,col.names=F)
      write.table(lambda_unc,file="lambda_cens_unc.txt",sep=" ",row.names=F,col.names=F)
      write.table(criterio_unc,file="criterio_cens_unc.txt",sep=" ",row.names=F,col.names=F)
      write.table(AIC_criterio_unc,file="AICcriterio_cens_unc.txt",sep=" ",row.names=F,col.names=F)
      write.table(dp_Infbf_unc,file="dp_Infbf_cens_unc.txt",sep=" ",row.names=F,col.names=F)
